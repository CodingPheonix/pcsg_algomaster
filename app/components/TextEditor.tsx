...
const TextEditor: React.FC<TextEditorProps> = ({ onSubmit, prevBlocks, taskId, prevTitle }) => {
    ...
    useEffect(() => {
        if (prevBlocks) setBlocks(prevBlocks)
        if (prevTitle) setTitle(prevTitle)
    }, [prevBlocks, prevTitle])
            ...
            <div className="sticky top-0 z-40 border-b backdrop-blur-xl">
                <div className="container mx-auto flex h-14 items-center justify-between px-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={handlePublish} className="rounded-lg bg-blue-500 px-4 py-1.5 text-xs font-mono font-medium text-white active:bg-blue-600 hover:bg-blue-500/90 transition-colors">
                            Publish
                        </button>
                    </div>
                </div>
